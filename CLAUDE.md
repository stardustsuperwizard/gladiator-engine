...
> above will revise **§5.2** (#377 — built: a player's Turns per round are one
> per champion, each activated once per round, replacing the authored
> `turnsPerPlayer`) and **§5.3** (#374 — a Turn becomes Pre-Action → Action →
> Post-Action). Neither section is settled; read both epics before building
> against either. `AGENTS.md` carries what each one changes and why the code is
