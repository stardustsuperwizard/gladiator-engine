...
> **Ported from `mikeys_game_bones-rules-moba` on 2026-09-04**
> (`EXTRACTION_LOG.md` #24). The control plane this describes — workflows,
> composite actions, agent profiles, classifier scripts — came over with it and
> lives in `.github/`.
>
> **Issue numbers in this document refer to the source repository, not to this
> one.** They are kept because they carry the reasoning behind decisions that
> were expensive to reach; treat them as citations to prior art, not as work
> items here. Anything describing the `sim/` Python balance harness or the
> `balance-*.yml` workflows describes something that was deliberately not
> ported — there is no Python harness in this repo.
>
> Before the control plane will do anything at all, run
> `.github/scripts/bootstrap-labels.sh`: every trigger here is keyed on a label
> name, and a fresh repository has none of them.
...
it delegates to. A planner that delegates to `implementer` in-session
...
| Implementer — native cloud agent | Claude Haiku 4.5 | The picker, when **you** dispatch — see *Four entry points* below. |
| Implementer — scripted (`agent:implementer:copilot`) | Per task, from the Issue's `model:*` label; Claude Opus 5, then Sonnet 5, then Haiku 4.5 when unlabelled | `model:*` label → `agent-02-implement.yml`'s *Resolve Implementer Model Tier*; default and override in `IMPLEMENTER_MODELS` env / `vars.IMPLEMENTER_MODELS`. See *The implementer's tier is chosen per task* below |
...
Planner, the scripted implementer, reviewer, and fixer are all Copilot CLI
...
`PLANNER_MODELS`, `IMPLEMENTER_MODELS`, `REVIEWER_MODELS`, and `FIXER_MODELS`
...
Implementer and fixer default to a cheaper tier instead — bounded,
...
session-wide, so the same list that catches an unavailable Haiku implementer also
...
outcome `IMPLEMENTER_MODELS` refuses when its `haiku` tier falls *up* through
...
- **These are Claude Code model IDs, and they use dashes.** `claude-opus-4-8`,
  not the `claude-opus-4.8` that appears in `IMPLEMENTER_MODELS` above — that is
...
implementer gets a starting point per Issue, because the tasks it runs are not
...
*Resolve Implementer Model Tier* step resolves the tier through a
...
decision, made once by the planner, read by whichever implementer the label
...
- **An operator outranks the planner.** Setting `vars.IMPLEMENTER_MODELS` is a
  deliberate decision about this repository; a planner's guess about one task
  does not overrule it. `vars.IMPLEMENTER_TIER_FLOOR` (default `haiku`) raises
  the lower bound instead, without editing the rubric or relabelling anything.
  `vars.CLAUDE_IMPLEMENTER_TIER_FLOOR` is its counterpart on the Claude
  pipeline. Both bind the implementer only: the fixers already have a floor in
...
**What this leans on.** A cheaper implementer is only safe because a strong
reviewer follows it — the reasoning this document already gives for the
implementer's tier being cheaper than the planner's. This change leans harder on
...
- **It only ever raises.** The base is `FIXER_BASE_TIER` (default `sonnet`),
  ...
  cheap implementer is a cost decision made before the code existed; the fixer is
...
the implementer side: if you name the list, you own it, including on the fourth
...
([run 32452540331](https://github.com/stardustsuperwizard/gladiator-engine/actions/runs/32452540331)).
...
review are paid for at all. Implementer and fixer default to the cheap tier
...
(`IMPLEMENTER_MODELS`) instead of a picker. There was once an
`agent:implementer:copilot`
label that dispatched the cloud agent blind onto Auto; that version was
removed. The current `agent:implementer:copilot` is a different mechanism
entirely — a
...
| --- | --- | --- | --- |
| **Desktop agents panel** — start a session | **your choice** | `implementer` | via the Run This Task block |
...
| `agent:implementer:copilot` label — scripted, not the cloud agent | preference list, led by the Issue's `model:*` tier (`IMPLEMENTER_MODELS` otherwise) | n/a — not a custom-agent session | yes, `Closes #n` written by the workflow itself |
...
`implementer` profile is worth nothing to a cloud session: its `tools:` list is
...
picker at all, that's what `agent:implementer:copilot` is for — it gives up
...
| `02-implementer.agent.md` | `Claude Haiku 4.5` |
...
1. **Claude Haiku 4.5 and Claude Opus 5 are both pickable** for the cloud
   ...
   availability list.
2. **Claude Sonnet 5 is in neither.** For the implementer the Anthropic choice is
...
Implementation Task
        │
        ├─────────────────────────────┬───────────────────────────┐
        │  you add                    │  you paste its "Run This  │
        │  agent:implementer:copilot  │  Task" block into the     │
        │  (scripted, no live picker) │  agents panel, model you  │
        │                             │  chose. (Assigning        │
...
└─────────────────────────────┘
        ...
        └──────────────┬──────────────┘
                ...
                PR ready for review
                        │
        │  you add agent:reviewer:copilot (auto only for the
...
└────────────────────────────────────────────────┘
   ...
   other → review:fix / planning-failure / design-ambiguity
        │
        │  review:fix?  you add  agent:fixer:copilot
        ▼
┌────────────────────────────────────────────────┐
...
scripted `agent:implementer:copilot` path follows the rule above like
everything else,
...
| `agent:implementer:copilot` label | You | `agent-02-implement.yml` | Implement this Task via the scripted CLI implementer — no live picker; walks a preference list, opens the PR, then self-reviews and self-fixes against it |
| `agent:reviewer:copilot` label | You | `agent-04-review.yml` | Re-review this PR |
...
implementer (`agent-02-implement.yml`) and the fixer (`agent-05-fix.yml`) are
...
```bash
gh secret delete PROJECT_TOKEN --repo stardustsuperwizard/gladiator-engine
```
...
If the run **fails**, it removes `agent:planner:copilot`, leaves `plan` in
place, and
comments with a link to the failed run. That is deliberate:
`agent:planner:copilot` is
...
and re-adding `agent:planner:copilot` is the same clean retry as always. Check
for
...
**The scripted implementer, `agent-02-implement.yml`.** Add the `agent:implementer:copilot`
...
2. Runs a Copilot CLI session (`IMPLEMENTER_MODELS`) against the Issue body plus
...
verdict, `review:*` label included — so adding `agent:reviewer:copilot` to it
is a
...
itself failing — comments on the Issue and removes `agent:implementer:copilot`
rather
...
check existed, a task expecting those paths ran a full implementer session on
...
- **The planner** marks such a task when it generates it — `human-credentials`
  ...
  that says the task is not runnable by the Copilot implementer and what to do
...
The implementer and the fixer ask the same question from opposite directions,
and the difference matters. The implementer runs before any diff exists, so it
...
The fixer's exposure is the one the implementer guard could not close. Since
the implementer now refuses a workflow-scoped Issue outright, no `agent-exec/*`
...
other. Its failure was also the worse of the two: the implementer's rejected
...
run passes. **A draft implementer PR is one no verified validation stands
behind.** Read the section, not the report.

This is load-bearing on one specific path. An implementer is told not to commit
...
marked ready for review, or any time you add **`agent:reviewer:copilot`**.
Only the
native cloud agent's branches match `copilot/*` — the scripted implementer's
...
converging. A human decides when to spend one, by adding
**`agent:fixer:copilot`**
...
and drop `agent:fixer:copilot` without starting a session. Neither is a red
run: a
...
independent check run an implementer push does — see *The independent
validation gate*.
...
Quote any label containing a colon. `label:agent:planner:copilot` is parsed as
a label
named `agent` qualified by a stray `plan`, and silently returns the wrong
set; `label:"agent:planner:copilot"` is unambiguous. Commas inside a single
`label:`
qualifier are OR.
...
orders by, what `agent:implementer:copilot` refuses to run past, and what
...
no blockers". A paid implementer session then runs against a task whose
...
automated path can push at all — see *Tasks the scripted implementer cannot
...
Status used to live on the [Gladiator Engine
...
out in the first place. `implementer` runs on Haiku; a cheap model given "fetch
...
filtering deterministic in the assembler rather than resting on reviewer
discipline.
...
It runs `/implementer` on the Issue, subscribes to the pull request that comes
...
- **The fixer's model climbs.** The first fix cycle runs at the tier the
  implementer built the code at — a model a tier below the one that wrote the
  ...
  configured model.
- **The implementer's model comes from the Issue.** `/implementer` reads the
  planner's `model:*` label and passes it as the subagent's model, which
  overrides `implementer.md`'s `model: haiku` frontmatter. An Issue with no tier
...
- **It does not add `agent:reviewer:copilot`.** That label is the entry to
...
the same GitHub connection, so the implementer subagent opens a pull request as
...
`agent:implementer:{vendor}` drives `agent-02-implement.yml`, which is the
single role, not the orchestrator. A headless run is bounded, the implementer
...
`agent:reviewer:copilot` by hand (works from GitHub Mobile) to put it through
the same
reviewer, same as the scripted implementer's own `agent-exec/*` branches
already require.
...
segment existed. They were kept working through the migration so nothing broke
mid-flight, then removed here — gates, retry messages and all — and deleted
from the repository.

Deleting rather than leaving them inert was the deliberate choice. A label that
exists and does nothing is a trap: someone adds it from a phone, sees no run
start, and has nothing to read. A label that does not exist cannot be added at
all, and the picker shows what replaced it.

Every message that used to say "re-add `agent:plan` to retry" now says to
re-add *the same label*, read from the run's own trigger. A retry therefore
re-runs the vendor that was asked for, and no message names a label that
cannot be applied.

**The eight labels already exist on the repository** — `agent:{planner,
implementer, reviewer, fixer}:{copilot, claude}` — created by hand rather than
...
- The prompt body caps at **30,000 characters**. All three files are well
  under: planner ~9k, implementer ~4.6k, reviewer ~1.5k.
...
| `.github/workflows/red-main.yml` | Triggered by `ci.yml` completion on `main`: fetches the run's state, calls `red-main.py`, and escalates a failing default branch by opening/updating/closing the single `red-main` Issue; credit-free, `GITHUB_TOKEN` only, never fixes |
...
| `.github/scripts/spec_traceability.py` | Parses the hex-skirmish-game-spec.md and validates the traceability index against the spec sections and filed modules |
...
| Copilot is assignable here | `gh api graphql -f query='{repository(owner:"stardustsuperwizard",name:"gladiator-engine"){suggestedActors(capabilities:[CAN_BE_ASSIGNED],first:20){nodes{login}}}}'` |
