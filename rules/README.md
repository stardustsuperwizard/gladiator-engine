...
The `## Layout` table above is a human overview mapping each directory to its spec section. The machine-readable form of the same mapping lives in `docs/spec-traceability.json`, indexed with stable `TR-NNNN` IDs and kept from rotting by `.github/scripts/test-spec-traceability.sh`. The table remains the human entry point and will not become a second source of truth.
