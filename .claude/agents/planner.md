...
# implementer and fixer both consume. That judgement should not be made by a
...
1. Fetch the Intake Issue and its comments. The `labels` field carries the
   ...
   ```bash
   # LOCAL
   gh issue view <n> --repo stardustsuperwizard/gladiator-engine \
     --json number,title,body,milestone,url,labels
   gh api --paginate repos/stardustsuperwizard/gladiator-engine/issues/<n>/comments
   ```
...
follow-up, the worst case is an Issue with no tier label — which the implementer
