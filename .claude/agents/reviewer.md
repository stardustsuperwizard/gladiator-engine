...
verdict for the implementer, not something you do yourself.
...
gh pr diff <pr-number> --repo stardustsuperwizard/gladiator-engine

gh api graphql -f owner=stardustsuperwizard -f name=gladiator-engine \
...
```bash
# LOCAL
gh issue view <task-number> --repo stardustsuperwizard/gladiator-engine \
...
```bash
# LOCAL
gh pr comment <pr-number> --repo stardustsuperwizard/gladiator-engine \
  --body-file <review-file>

# clear the other three first, then add the one that applies
gh pr edit <pr-number> --repo stardustsuperwizard/gladiator-engine \
