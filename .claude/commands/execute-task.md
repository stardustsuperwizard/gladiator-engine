...
stardustsuperwizard/gladiator-engine all the way from an
...
`/planner`, `/implementer`, `/reviewer`, `/fixer` — each run one agent once and
...
```text
/implementer -> subscribe -> CI -> /reviewer -> /fixer -> back to CI
                                ^                         |
                                +----- up to 4 cycles -----+
```

Each numbered step below invokes a role command rather than restating its
contract. Invoke them through the `Skill` tool by name — `implementer`,
...
Invoke the `implementer` command with the Issue number.

It runs the Issue guards, reads the `model:*` label, delegates to the
`implementer` subagent at that tier, and reports a PR URL. Note the tier it
...
which is where `/implementer` got it and which is still there. An Issue with no
tier label resolves to `sonnet`, by the same rule `/implementer` applies. This
...
```bash
gh run view <run-id> \
  --repo stardustsuperwizard/gladiator-engine --log-failed
```
...
The implementer already ran `.github/scripts/validate-godot.sh` locally, and
...
```bash
# LOCAL
gh pr view <pr-number> --repo stardustsuperwizard/gladiator-engine \
  --json labels --jq '[.labels[].name | select(startswith("review:"))]'
```

```text
CLOUD — call mcp__github__issue_read with method="get_labels",
owner="stardustsuperwizard", repo="gladiator-engine",
...
| Fix cycle | Model |
| --- | --- |
| 1st | the tier the implementer ran at, recorded in step 1 |
| 2nd, 3rd, 4th | `opus` |

The first cycle matches the implementer because the fix is a correction to that
implementer's own work, in a codebase it just built. A model a tier below the
...
the implementer ran at `haiku` or `sonnet` — that is the case the rule exists
for. If the implementer already ran at `opus`, there is nothing to escalate to
...
implementer's tier always clears it. If you ever raise that frontmatter, the
