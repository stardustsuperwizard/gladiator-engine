...
Review pull request #$ARGUMENTS in stardustsuperwizard/gladiator-engine.
...
```bash
# LOCAL
gh pr view $ARGUMENTS --repo stardustsuperwizard/gladiator-engine \
  --json number,title,body,url,headRefName,isDraft,files
gh pr diff $ARGUMENTS --repo stardustsuperwizard/gladiator-engine
```
