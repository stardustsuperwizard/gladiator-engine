---
description: Implement one Implementation Task Issue and open the PR that closes it (local counterpart of the agent:implementer:copilot label)
argument-hint: <task-issue-number>
---

Implement Implementation Task Issue #$ARGUMENTS in
stardustsuperwizard/gladiator-engine.

This command runs the `implementer` subagent once and stops at the open PR. It
...
```bash
# LOCAL only
gh issue view $ARGUMENTS --repo stardustsuperwizard/gladiator-engine --json blockedBy
```
...
in its *Resolve Implementer Model Tier* step, so the CI path reaches this
...
reads it back, and "whatever the implementer used" has to mean something
specific.

## Run it

Delegate to the `implementer` subagent, giving it the Issue number, the fetched
...
```bash
# LOCAL
gh pr list --repo stardustsuperwizard/gladiator-engine \
...
Report the implementer's completion report, the PR URL, and the model tier you
